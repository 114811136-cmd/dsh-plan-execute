...
async function askFixStrategy(ctx, invocation, failed, problems) {
...
function collectFailed(results, plan) {
...
async function run(ctx, config, invocation, childEfforts) {
  ...
  try {
    ...
    for (let round = 0; round < config.maxFixRounds; round += 1) {
      ...
      const needsFix = failed.length > 0 || verdict !== void 0 && !verdict.pass;
      if (!needsFix) break;
      if (signal.aborted) return { kind: "error", text: "\u5DF2\u505C\u6B62" };
      ...
      const retryTasks = plan.tasks.filter((task2) => retryIds.has(task2.task_id)).map((task2) => modifyText.length > 0 ? { ...task2, description: `${task2.description}
\u3010\u7528\u6237\u4FEE\u6539\u8981\u6C42\u3011${modifyText}` } : task2);
