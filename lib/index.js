...
async function askWorkshop(ctx, invocation, plan, results) {
  ...
  const taskOptions = results.filter((result) => result !== void 0).map((result) => ({
...
async function run(ctx, config, invocation, childEfforts) {
  ...
  try {
    ...
    for (let round = 0; round < config.maxFixRounds; round += 1) {
      ...
      const retryTasks = plan.tasks.filter((task2) => decision.modifyTaskIds.includes(task2.task_id)).map((task2) => ({
        ...task2,
