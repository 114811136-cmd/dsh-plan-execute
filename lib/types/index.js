    ...
    const workers = Array.from({ length: workerCount }, async () => {
        for (;;) {
            ...
            for (let i = 0; i < ordered.length; i += 1) {
                ...
                if (candidate === undefined || dispatched.has(candidate.task_id))
                    continue;
...
function effortForModel(config, model) {
    if (model === undefined)
        return undefined;
    if (model === config.executorModel)
        return config.executorEffort;
    if (model === config.reviewerModel)
        return config.reviewerEffort;
    if (model === config.plannerModel)
        return config.plannerEffort;
    return undefined;
}
export function apply(ctx, config) {
    ...
    ctx.on('agent/request', async (payload, next) => {
        ...
        if (!childSessions.has(payload.agent.id))
            return resolved;
